      ...
      <div className="relative z-10 w-full max-w-[1200px] grid lg:grid-cols-2 min-h-screen lg:min-h-[auto] lg:rounded-3xl lg:border border-white/10 lg:bg-black/40 lg:backdrop-blur-xl lg:shadow-2xl overflow-hidden my-auto">
        ...
        <div className="hidden lg:flex flex-col justify-between p-12 relative border-r border-white/5">
          ...
          <div className="relative z-10 pointer-events-none space-y-4">
            <p className="font-heading text-3xl font-bold leading-tight max-w-md text-white">Join 40,000+ makers who ship faster with AI-powered planning.</p>
            <div className="text-xs text-[#A1A1AA] font-mono">Free tier forever · No credit card required</div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center p-8 bg-black/20 backdrop-blur-md lg:bg-transparent lg:backdrop-blur-none">
          <form onSubmit={submit} className="w-full max-w-sm space-y-6" data-testid="register-form">
            <div>
              ...
              <p className="mt-2 text-sm text-[#A1A1AA]">Start organising your work with AI, in 30 seconds.</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input id="password" type="password" value={password} onChange={(e)=>setPassword(e.target.value)} required minLength={6} data-testid="register-password" />
                <p className="mt-1 text-xs text-[#A1A1AA]">At least 6 characters.</p>
              </div>
            </div>

            {error && <div className="text-sm text-rose-400 font-mono" data-testid="register-error">{error}</div>}

            <Button type="submit" disabled={loading} className="w-full rounded-full bg-white text-black hover:bg-zinc-200 font-semibold" data-testid="register-submit">
              {loading ? "Creating…" : "Create account"}
            </Button>
            <p className="text-sm text-[#A1A1AA] text-center">
              Already have an account? <Link to="/login" className="text-[#7158F5] hover:underline">Sign in</Link>
            </p>
          </form>
        </div>
      </div>
